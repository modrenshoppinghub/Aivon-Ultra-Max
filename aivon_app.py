...
with st.sidebar:
    ...
    groq_api = st.text_input("🔑 Groq API Key", type="password", help="Enter your Groq key to power the AI Brain.")
...
with col_main[1]:
    topic = st.text_input("🌐 Strategic Vision Input:", placeholder="Enter a global trend, industry concept, or viral story theme...")
...
search_tool = DuckDuckGoSearchRun()
            ...
            task1 = Task(description=f"Research 3 breakthrough insights for {topic}.", expected_output="Intelligence Brief.", agent=researcher)
            task2 = Task(description="Write a 60-second viral video script with scene descriptions.", expected_output="Production Script.", agent=director)
        ...
        with res_col1:
            ...
            st.download_button("📥 Download PDF Report", data=str(result.raw), file_name="Aivon_Report.txt")

        with res_col2:
            st.markdown("### 🖼️ Neural Visual")
            st.image(image_url, caption=f"Generated for: {topic}", use_container_width=True)
            st.success("4K Asset Ready for Export")

# --- 5. EARNINGS / PRICING SECTION ---
st.divider()
